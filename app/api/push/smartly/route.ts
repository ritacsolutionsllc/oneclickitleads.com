...
export async function POST(req: NextRequest) {
  ...
  // Quality-first gate: smartly.io Custom Audiences only get export-eligible
  // rows. This prevents raw scraped-only leads from ever reaching paid
  // activation, regardless of whether the caller sets a min_score.
  let q = supabase
    .from('leads')
    .select('email, phone_e164, first_name, last_name, city, region, country, quality_score')
