...
export async function GET(req: NextRequest) {
  ...
  // Quality gate: only rows explicitly marked eligible by the scoring engine.
  // `is_scrubbed = true` is still checked as defense in depth against legacy
  // rows inserted before migration 0005 landed.
  let q = supabase
    .from('leads')
    .select('email, first_name, last_name, phone_e164, company, title, icp_segment, city, region, country, scrub_score, quality_score, verification_status, source_tier')
